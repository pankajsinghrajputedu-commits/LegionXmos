...
import { TrendingUp, CheckCircle, Loader2, Search } from 'lucide-react';
      ...
      <div className="min-h-screen bg-gray-50 p-4 lg:p-8">
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="space-y-2">
                <div className="text-xs font-semibold text-gray-500 uppercase">Candidate</div>
                <div className="text-lg font-semibold text-gray-900">{selectedScore.candidate_name}</div>
                <div className="text-sm text-gray-600">{selectedScore.candidate_email}</div>
              </div>

              <div className="space-y-2">
                <div className="text-xs font-semibold text-gray-500 uppercase">Total Score</div>
                <div className="text-4xl font-bold text-red-800">{selectedScore.total_score.toFixed(1)}</div>
                <div className="text-sm text-gray-600">out of {selectedScore.question_scores.length * 10}</div>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
            ...
            <div className="space-y-4">
              {selectedScore.question_scores.map((qs, idx) => (
                <div key={qs.question_id} className="p-4 rounded-lg bg-gray-50 border border-gray-200 space-y-3">
                  <div className="flex items-center justify-between">
                  ...
                  <p className="text-sm text-gray-600">{qs.reasoning}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
            ...
            <p className="text-gray-700 leading-relaxed">{selectedScore.recommendations}</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 lg:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">AI Scoring</h1>
          <p className="text-gray-600">View detailed AI evaluations for all candidate submissions</p>
        </div>
            ...
            {filteredSubmissions.map((submission) => {
              ...
              return (
                <div
                  ...
                  className={`bg-white rounded-xl shadow-sm border border-gray-200 p-6 ${score ? 'cursor-pointer hover:border-red-300 hover:shadow-md' : ''} transition-all`}
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="font-semibold text-gray-900 text-lg">{submission.candidate_name}</div>
                        <div className="text-sm text-gray-600">{submission.candidate_email}</div>
